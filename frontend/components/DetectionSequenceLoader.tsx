 ...
 *
 * Ported from main and adapted to this branch's brand hex palette
 * (moss green #2BEE34 / dark neutral #141414) instead of main's
 * bg-primary/text-text-muted design-token classes.
 */
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="relative w-10 h-10 flex items-center justify-center">
          <div className="w-10 h-10 rounded-xl bg-[#2BEE34]/10 flex items-center justify-center">
            <AnimatePresence mode="wait">
              <motion.div
                key={active?.id}
                ...
                transition={{ duration: 0.2 }}
              >
                {active && <active.icon className="w-5 h-5 text-[#2BEE34]" />}
              </motion.div>
            </AnimatePresence>
          </div>
          {!reduced && (
            <motion.div
              className="absolute inset-0 rounded-xl border border-[#2BEE34]/30"
      ...
      <div className="space-y-1.5">
        <div className="flex justify-between items-center">
          ...
          <span className="text-xs font-bold text-[#2BEE34] tabular-nums ml-2 shrink-0">
            {overallPct}%
          </span>
        </div>
        <div className="h-1.5 bg-[#1E1E1E] rounded-full overflow-hidden">
          <motion.div
            className="h-full rounded-full bg-[#2BEE34]"
      ...
      <div className="space-y-1.5">
        ...
        {completed.map((s, i) => (
          <motion.div
            ...
            className="flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg bg-[#2BEE34]/5 border border-[#2BEE34]/15"
          >
            <div className="w-6 h-6 rounded-lg bg-[#2BEE34]/10 border border-[#2BEE34]/25 flex items-center justify-center shrink-0">
              <CheckCircle2 className="w-3.5 h-3.5 text-[#2BEE34]" />
            </div>
            <span className="text-xs text-[#2BEE34] font-medium">{s.done}</span>
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="ml-auto text-[10px] font-bold text-[#2BEE34]/70"
            >
              ✓
            </motion.div>
          </motion.div>
        ...
        {pending.slice(0, 3).map(s => (
          ...
          >
            <div className="w-6 h-6 rounded-lg bg-[#1E1E1E]/60 border border-[#1E1E1E] flex items-center justify-center shrink-0">
              <s.icon className="w-3.5 h-3.5 text-[#6B6B6B]" />
            </div>
      ...
      <div className="flex items-center gap-2 text-[11px] text-[#6B6B6B]">
        <div className={`w-1.5 h-1.5 rounded-full bg-[#2BEE34] ${!reduced ? 'animate-pulse' : ''}`} />
        {step < STEPS.length - 2
