'use client'
/**
 * ClientSections — all interactive, client-side content for the homepage.
 * Extracted from app/page.tsx to unblock LCP on the static hero.
 * Loaded via next/dynamic with ssr: false — does NOT block the hero from painting.
 *
 * Contains:
 *   - Fixed NAV (position: fixed — DOM order does not affect visual placement)
 *   - Hero overlay (FloatingCards, NetworkBackground, animated badge, LiveDemo)
 *   - All below-fold sections (Stats, AIvsReal, Tools, HowItWorks, Reviews, Trust, CTA)
 *   - Site Footer
 */
...
const DynamicWhoNeedsSection = dynamic(
  ...
  { ssr: false, loading: () => <div className="min-h-[560px] sm:min-h-[700px] bg-background" aria-hidden="true" /> }
)
const DynamicAIvsRealSection = dynamic(
  () => import('@/components/home/AIvsRealSection'),
  { ssr: false, loading: () => <div className="min-h-[320px] sm:min-h-[420px] bg-background" aria-hidden="true" /> }
)
    ...
    <div className="relative">
      <div className="rounded-2xl border border-purple-500/20 bg-surface p-4 sm:p-5 shadow-2xl shadow-purple-500/5">
          ...
          <div className="flex gap-2">
            {examples.map(ex => (
              <button key={ex.label} onClick={() => setText(ex.text)}
                ...
                className="text-xs px-2.5 py-1 rounded-lg border border-border hover:border-purple-500/40 text-text-muted hover:text-purple-400 transition-colors duration-200">
                {ex.label}
              </button>
            ))}
          </div>
        ...
        <textarea value={text} onChange={e => setText(e.target.value)}
          ...
          className="w-full min-w-0 h-24 sm:h-28 bg-background/80 border border-border rounded-xl px-3 sm:px-4 py-3 text-sm text-text-primary placeholder:text-text-muted resize-none focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/20 transition-colors duration-200" />
...
function HeroScrollIndicator() {
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={-1}
      className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1 animate-bounce opacity-30 pointer-events-none"
    >
...
export default function ClientSections() {
  ...
  // iOS scroll lock while mobile nav is open
  useEffect(() => {
    if (mobileNavOpen) {
  ...
  return (
    <>
      {/* ══ NAV — position:fixed, DOM order irrelevant for visual placement ══ */}
      <nav className={`fixed top-0 left-0 right-0 z-50 h-16 transition-[background-color,border-color,box-shadow,transform] duration-300
      ...
      {/* ══ HERO OVERLAY — animated elements over the static hero ══ */}
      {/* This positions over the first 100svh hero section rendered in page.tsx */}
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 5, height: '100svh' }} aria-hidden="true">
        <NetworkBackground />
        <FloatingCards />
        ...
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] sm:w-[700px] sm:h-[700px] md:w-[900px] md:h-[900px] rounded-full pointer-events-none will-change-transform"
          style={{
      ...
      {/* Hero interactive overlay: animated badge, CTAs with auth, LiveDemo, scroll indicator */}
      {/* Uses absolute positioning to overlay the static hero */}
      <div className="absolute top-0 left-0 right-0 min-h-[100svh] flex items-center justify-center pointer-events-none pt-16 sm:pt-28 lg:pt-32 pb-12 sm:pb-20" style={{ zIndex: 20 }}>
        <div className="relative text-center px-4 sm:px-6 md:px-8 lg:px-4 max-w-7xl mx-auto w-full">
          {/* Animated badge replaces static one after JS loads */}
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
            className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full border border-purple-500/30 bg-purple-500/10 text-purple-300 text-[11px] sm:text-xs font-semibold mb-3 sm:mb-7 pointer-events-auto">
          ...
          {/* Spacer for HeroHeadline height */}
          <div className="mb-4 sm:mb-7 opacity-0 pointer-events-none select-none" aria-hidden="true">
            <div className="h-16 sm:h-20 lg:h-24" />
          </div>

          {/* Spacer for subtitle */}
          <div className="mb-7 sm:mb-10 opacity-0 pointer-events-none select-none" aria-hidden="true">
            <div className="h-16 sm:h-12" />
          </div>

          {/* Auth-aware CTAs overlay */}
          <motion.div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 mb-9 sm:mb-14 pointer-events-auto"
            initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}>
            {user ? (
              <>
                <Link href="/dashboard"
                  className="group relative w-full sm:w-auto px-8 py-4 rounded-2xl text-white text-base font-bold flex items-center justify-center gap-3 overflow-hidden transition-[transform,box-shadow] duration-200 hover:scale-[1.02] active:scale-[0.98]"
                ...
                <Link href="/chat" className="w-full sm:w-auto px-8 py-4 rounded-2xl border border-border/60 bg-surface text-base font-semibold flex items-center justify-center gap-2 hover:border-purple-500/40 transition-[border-color,background-color] duration-200">
                  <MessageSquare className="w-5 h-5 text-emerald" />ARIA Assistant
                </Link>
              </>
            ) : (
              // Invisible placeholder — static page.tsx CTAs are already visible
          ...
          {/* Live demo */}
          <motion.div className="max-w-2xl 2xl:max-w-3xl mx-auto w-full pointer-events-auto"
      ...
      {/* ══ BELOW-FOLD SECTIONS ══ */}
      {/* Spacer so below-fold content starts below the hero */}
        ...
        <div className="max-w-5xl mx-auto relative">
          ...
          <div className="text-center mt-10 flex flex-col sm:flex-row items-center justify-center gap-3">
            <a href="mailto:contact@aiscern.com"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-purple-500/30 bg-purple-500/8 text-sm font-semibold text-purple-400 hover:bg-purple-500/15 transition-[background-color] duration-200">
              Share Your Feedback
            </a>
              ...
              return (
                <motion.div key={title}
                  ...
                  <SpotlightCard color="rgba(139,92,246,0.10)"
                    className={`h-full p-6 sm:p-7 rounded-2xl border border-border/60 bg-gradient-to-br ${bg} hover:border-purple-500/25 transition-[border-color,box-shadow] duration-300 ${large ? 'bento-shimmer' : ''}`}>
      ...
      <section className="py-10 sm:py-14 border-t border-border/15 overflow-hidden relative">
        <div className="relative">
          ...
          <div className="marquee-track marquee-left py-2">
            {[...PROFESSIONALS, ...PROFESSIONALS].map((prof, i) => (
