...
export const metadata: Metadata = {
  ...
  description: 'Step-by-step guides on how to verify AI text, verify deepfake images, check audio for voice cloning, and more. Free resources for all users.',
  openGraph: {
    title: 'User Guides — Aiscern',
...
const GUIDES = [
  {
    icon: FileType2,
    color: 'primary',
    title: 'How to Verify AI-Generated Text',
    desc: 'A step-by-step guide to identifying AI-written content using Aiscern\'s text verification ensemble. Covers paste, upload, URL examination, and interpreting integrity ratings.',
    steps: ['Paste or upload your text', 'Review overall confidence score', 'Examine sentence-level heatmap', 'Export report if needed'],
    href: '/detect/text',
    cta: 'Try Text Verification',
  },
  {
    icon: Eye,
    color: 'blue',
    title: 'How to Verify Deepfake Images',
    desc: 'Learn how to upload images for AI generation verification, understand the pixel-integrity analysis, and interpret results for synthetic images from Midjourney, DALL-E, Stable Diffusion, and more.',
    ...
    href: '/detect/image',
    cta: 'Try Image Verification',
  },
  {
    icon: Music,
    color: 'emerald',
    title: 'How to Verify AI-Cloned Audio',
    desc: 'Understand how Aiscern analyzes audio files for voice synthesis signatures. Guide covers supported formats, spectral analysis, and interpreting voice clone verification results.',
    ...
    href: '/detect/audio',
    cta: 'Try Audio Verification',
  },
  {
    icon: Video,
    color: 'amber',
    title: 'How to Verify Deepfake Video',
    desc: 'A complete guide to video deepfake verification using Aiscern\'s frame-level ViT ensemble and temporal consistency analysis. Covers face-swap, lip-sync, and full-body synthesis verification.',
  ...
  },
  {
    ...
    desc: 'A deep dive into what Aiscern\'s confidence percentages actually mean, how to interpret uncertain results, and when not to act on a single verification result.',
...
const colorMap: Record<string, { bg: string; border: string; icon: string; badge: string }> = {
  primary: { bg: 'bg-accent/10', border: 'border-accent/20', icon: 'text-accent', badge: 'text-accent bg-accent/10 border-accent/20' },
  blue:    { bg: 'bg-accent/10', border: 'border-accent/20', icon: 'text-accent', badge: 'text-accent bg-accent/10 border-accent/20' },
  cyan:    { bg: 'bg-accent/10', border: 'border-accent/20', icon: 'text-accent', badge: 'text-accent bg-accent/10 border-accent/20' },
  emerald: { bg: 'bg-accent/10', border: 'border-accent/20', icon: 'text-accent', badge: 'text-accent bg-accent/10 border-accent/20' },
  amber:   { bg: 'bg-warning/10',   border: 'border-warning/20',   icon: 'text-warning',   badge: 'text-warning bg-warning/10 border-warning/20'   },
  rose:    { bg: 'bg-error/10',    border: 'border-error/20',    icon: 'text-error',    badge: 'text-error bg-error/10 border-error/20'     },
}
        ...
        <section className="pt-10 pb-12 md:py-24 relative overflow-hidden">
          ...
          <div className="max-w-4xl 2xl:max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 2xl:px-12 text-center relative">
            ...
            <p className="text-base sm:text-lg text-silver-700 max-w-2xl mx-auto">
              Step-by-step guides for every verification modality. Learn how to get the most accurate results and interpret what the scores mean.
            </p>
          </div>
        </section>
          ...
          <div className="max-w-5xl 2xl:max-w-[1300px] mx-auto px-4 sm:px-6 lg:px-8 2xl:px-12">
            <div className="grid sm:grid-cols-2 gap-6">
              {GUIDES.map((guide, i) => {
                ...
                return (
                  <div key={i} className="card border border-silver-300 rounded-xl p-6 flex flex-col gap-5">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-silver-600 leading-relaxed">{guide.desc}</p>
                      </div>
                    </div>
                    ...
                    <ol className="space-y-2">
                      {guide.steps.map((step, j) => (
                        <li key={j} className="flex items-start gap-2.5 text-xs text-silver-700">
            ...
            <div className="mt-12 p-6 rounded-xl border border-accent/20 bg-accent/5 text-center">
              ...
              <p className="text-sm text-silver-600 mb-4">Check our FAQ, read the API docs, or contact our support team.</p>
