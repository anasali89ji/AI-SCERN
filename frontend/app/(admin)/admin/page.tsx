  ...
  return (
    <div className="rounded-xl border border-white/[0.08] bg-surface/60 p-4 flex items-start gap-3">
      ...
      <div className="min-w-0">
        ...
        {loading ? (
          <div className="h-6 w-16 bg-[#141420] animate-pulse rounded mt-1" />
        ) : (
          <p className="text-xl font-black text-slate-100 mt-0.5">{value}</p>
        )}
...
function MaintenancePanel() {
  ...
  return (
    <div className={`rounded-xl border p-4 ${enabled ? 'border-rose/30 bg-rose/5' : 'border-border bg-surface/60'}`}>
      <div className="flex items-center justify-between gap-3 mb-3">
        ...
        <button
          ...
          className={`relative w-12 h-7 rounded-full transition-colors shrink-0 disabled:opacity-50
            ${enabled ? 'bg-rose' : 'bg-surface-active'}`}
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <input
          ...
          className="flex-1 min-w-0 h-10 rounded-lg border border-border bg-surface px-3 text-[13px] text-text-primary
                     placeholder:text-text-disabled outline-none focus:border-primary"
        />
        <button
          onClick={saveMessage}
          disabled={saving}
          className="h-10 px-4 rounded-lg border border-border hover:bg-surface-active transition-colors
                     text-[12px] font-semibold text-text-primary disabled:opacity-50 shrink-0"
        >
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-slate-600">Last updated: {lastRefresh.toLocaleTimeString()}</span>
        ...
        {/* Maintenance Mode — the toggle that never actually wrote to the
            database (see app/api/admin/maintenance/route.ts comment). */}
        <MaintenancePanel />
        ...
        <div className="grid lg:grid-cols-5 gap-4">
            ...
            <div className="space-y-1 max-h-72 overflow-y-auto">
              {loading ? (
                Array.from({ length: 8 }).map((_, i) => (
                  <div key={i} className="h-8 bg-[#141420] animate-pulse rounded" />
                ))
              ) : (stats?.recentSessions || []).length === 0 ? (
                <p className="text-xs text-slate-500 text-center py-8">No sessions tracked yet</p>
              ) : (
                (stats?.recentSessions || []).map((s: any, i: number) => (
                  <div key={i} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-[#141420]/50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] text-slate-100 truncate">{s.email || s.user_id?.slice(0, 12)}</p>
                      <p className="text-[9px] text-slate-500">{s.page || '/'} · {s.ip}</p>
                    </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="rounded-xl border border-white/[0.08] bg-surface/60 p-4">
              <h3 className="text-sm font-bold text-slate-100 mb-3">Detection Accuracy</h3>
            ...
            {/* Recent Signups */}
            <div className="rounded-xl border border-white/[0.08] bg-surface/60 p-4">
              <h3 className="text-sm font-bold text-slate-100 mb-3">Recent Signups</h3>
              <div className="space-y-1.5">
                {(stats?.recentSignups || []).slice(0, 5).map((u: any, i: number) => (
                  <div key={i} className="flex items-center gap-2">
                    <div className="w-5 h-5 rounded-full bg-blue-500/10 flex items-center justify-center text-[9px] font-bold text-blue-400 flex-shrink-0">
                      {(u.email || 'U').charAt(0).toUpperCase()}
                    </div>
                    <span className="text-[11px] text-slate-500 truncate flex-1">{u.email}</span>
                    <span className="text-[9px] text-slate-600 whitespace-nowrap">
                ...
                {!loading && !stats?.recentSignups?.length && (
                  <p className="text-xs text-slate-500 text-center py-3">No signups yet</p>
                )}
              </div>
            </div>
          </div>
        ...
        {modalityChart.length > 0 && (
          <div className="rounded-xl border border-white/[0.08] bg-surface/60 p-4">
            <h3 className="text-sm font-bold text-slate-100 mb-3">Scans by Modality</h3>
