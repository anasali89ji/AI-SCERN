...
function StatusBadge({ user }: { user: User }) {
  if (user.is_banned)         return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#FF4444]/10 text-[#FF4444] border border-[#FF4444]/20">Banned</span>
  if (!user.dashboard_access) return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#FFB800]/10 text-[#FFB800] border border-[#FFB800]/20">Revoked</span>
  return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#2BEE34]/10 text-[#2BEE34] border border-[#2BEE34]/20">Active</span>
}
      ...
      <div className="p-6 2xl:p-8 max-w-7xl 2xl:max-w-[1600px] 3xl:max-w-[1900px] mx-auto">
        ...
        <div className="flex gap-1 mb-5 p-1 bg-[#0f0f17] border border-white/[0.08] rounded-xl w-fit">
          ...
          >
            ...
            Upgrade Requests
            {pendingCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] px-1 bg-[#FFB800] text-black text-[9px] font-black rounded-full flex items-center justify-center">
                        ...
                        <div className="flex items-center gap-1 flex-wrap">
                            ...
                            <>
                              {u.plan_granted_by && (
                                <button
                                  ...
                                  className="flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-semibold bg-[#FF4444]/10 text-[#FF4444] border border-[#FF4444]/20 hover:bg-[#FF4444]/20 transition-colors"
                                >
                          ...
                          {u.dashboard_access ? (
                            <button
                              ...
                              className="flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-semibold bg-[#FFB800]/10 text-[#FFB800] border border-[#FFB800]/20 hover:bg-[#FFB800]/20 transition-colors"
                            >
        ...
        {tab === 'requests' && (
          <>
            <div className="flex items-start gap-3 p-4 mb-4 rounded-xl bg-[#FFB800]/5 border border-[#FFB800]/20">
              <ArrowUpCircle className="w-4 h-4 text-[#FFB800] mt-0.5 flex-shrink-0" />
              <div>
              ...
              {['pending', 'approved', 'rejected', 'all'].map(s => (
                ...
                >
                  ...
                  {s === 'pending' && pendingCount > 0 && (
                    <span className="ml-0.5 px-1.5 py-0.5 rounded bg-[#FFB800]/20 text-[#FFB800] text-[9px] font-black">{pendingCount}</span>
                  )}
                </button>
              ))}
            </div>
                        ...
                        {r.status === 'pending' ? (
                          <div className="flex items-center gap-1.5">
                            <button
                              ...
                              className="flex items-center gap-1 px-2.5 py-1.5 rounded-md text-[10px] font-bold bg-[#2BEE34]/10 text-[#2BEE34] border border-[#2BEE34]/20 hover:bg-[#2BEE34]/20 transition-colors"
                            >
          ...
          <div className="bg-surface border border-white/[0.08] rounded-xl p-6 w-full max-w-sm ">
            <div className="flex items-start gap-3 mb-4">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                ...
                modal.action === 'revoke_pro' ? 'bg-[#FF4444]/10' :
                modal.action === 'ban'        ? 'bg-[#FF4444]/10' :
                modal.action === 'reset_daily'? 'bg-slate/10' : 'bg-[#FFB800]/10'
              }`}>
              ...
              <button onClick={doAction} disabled={!!actionLoading}
                className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-colors disabled:opacity-50 ${
                  ['ban', 'revoke', 'revoke_pro'].includes(modal.action)
                    ? 'bg-rose-500 text-white hover:bg-[#FF4444]/90'
                    : modal.action === 'grant_pro' || modal.action === 'set_plan'
                    ? 'bg-blue-600 text-white hover:bg-blue-700'
                    : 'bg-[#2BEE34]/90 text-white hover:bg-emerald-500'
                }`}>
