...
// Shared appearance so /login and /signup never drift apart. The `card`
// element is transparent/borderless/unrounded on purpose — the outer
// `<div>` in the JSX below is the single frame that owns the border and
// radius. Previously the header (`border-b-0`) and the Clerk card
// (`border-t-0`) were two separately-bordered boxes stacked on top of each
// other, which is what produced the split/uneven perimeter.
    ...
    <div className="min-h-screen flex flex-col lg:flex-row">
      ...
      <div className="lg:w-1/2 bg-surface-deep flex flex-col items-center justify-center px-4 py-10 lg:py-0">
        <div className="w-full max-w-[420px]">
          ...
          {/* Single unified frame — header and Clerk content share ONE
              border/radius. This is the fix: previously the header and the
              Clerk card were two separately-bordered elements stacked
              together (border-b-0 on top, border-t-0 below), which produced
              a visibly split/uneven perimeter. Now there is exactly one
              outer box, and a hairline divider (not a second border)
              separates the header from the form. */}
          <div className="bg-surface-deep border border-white/20 rounded-xl overflow-hidden">
            ...
            {/* Clerk widget — appearance keeps its own card transparent and
                borderless so the outer div above is the only visible frame. */}
            <SignIn
              routing="path"
              path="/login"
