...
export default function PipelinePage() {
  ...
  return (
    <div className="p-4 sm:p-6 2xl:p-8 space-y-6 max-w-5xl 2xl:max-w-[1300px] 3xl:max-w-[1600px] mx-auto">
      ...
      {pushMsg && (
        <div className={`p-4 rounded-xl text-sm font-medium border ${pushMsg.startsWith('✅') ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border-rose-500/20 text-rose-400'}`}>
          {pushMsg}
        </div>
      )}
              ...
              {workers.length > 0 ? workers.map(w => (
                <div key={w.num} className="flex items-center justify-between p-3 bg-background/60 rounded-xl">
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full ${w.online ? 'bg-emerald-500' : 'bg-rose-500'}`} />
                    <div className="flex-1 min-w-0">
                  ...
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${w.online ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
