...
import {
  Globe, Search, Shield, AlertTriangle, CheckCircle, XCircle,
  Image as ImageIcon, FileText, Layers, TrendingUp, Clock,
  ChevronDown, ChevronUp, ExternalLink, Lock, Eye,
  Zap, BarChart3, Fingerprint, Cpu, Sparkles, Loader2
} from 'lucide-react'
...
export default function ScannerPage() {
  ...
  function getVerdictBg(verdict: string) {
    switch (verdict) {
      case 'AI': return 'bg-rose-500/10 border-rose-500/30'
      case 'HUMAN': return 'bg-emerald-500/10 border-emerald-500/30'
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 pb-24 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">Deep AI detection across text, images, and site structure.</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-white/[0.07] bg-[#0f0f17] p-4">
        ...
        {error && (
          <div className="mt-3 p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-sm flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span className={`text-xs px-2 py-0.5 rounded-lg font-medium ${
              page.verdict === 'AI' ? 'bg-rose-500/20 text-rose-400' :
              page.verdict === 'HUMAN' ? 'bg-emerald-500/20 text-emerald-400' :
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className={`text-xs px-2 py-0.5 rounded-lg font-medium ${
              image.verdict === 'AI' ? 'bg-rose-500/20 text-rose-400' :
              image.verdict === 'HUMAN' ? 'bg-emerald-500/20 text-emerald-400' :
              'bg-amber-500/20 text-amber-400'
            }`}>
              {image.verdict}
            </span>
          ...
          <div className="flex items-center gap-3 mt-1">
            <span className={`text-lg font-bold ${image.aiScore > 0.6 ? 'text-rose-400' : image.aiScore < 0.4 ? 'text-emerald-400' : 'text-amber-400'}`}>
              {(image.aiScore * 100).toFixed(0)}%
            </span>
            <button onClick={onToggle} className="text-slate-600 hover:text-slate-300">
              {expanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
            </button>
        ...
        <div className="mt-3 space-y-2 border-t border-white/[0.07] pt-3">
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="p-2 bg-[#0f0f17] rounded-lg">
              <span className="text-slate-600">ELA Uniformity</span>
              <div className="font-mono text-slate-300">{image.elaUniformity}</div>
            </div>
            <div className="p-2 bg-[#0f0f17] rounded-lg">
              <span className="text-slate-600">DCT Anomaly</span>
              <div className="font-mono text-slate-300">{image.dctAnomaly}</div>
            </div>
          </div>
          {image.exifFlags.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                {image.exifFlags.map((flag, i) => (
                  <span key={i} className="text-xs bg-[#0f0f17] text-rose-400 px-2 py-1 rounded-lg border border-white/5">
                    {flag}
                  </span>
                ))}
              </div>
...
function RemediationCard({ item }: { item: RemediationItem }) {
  const borderClass = item.priority === 'critical' ? 'border-rose-500/30 bg-rose-500/5' :
    item.priority === 'high' ? 'border-orange-500/30 bg-orange-500/5' :
    item.priority === 'medium' ? 'border-amber-500/30 bg-amber-500/5' :
    'border-white/5 bg-[#141420]'

  const textClass = item.priority === 'critical' ? 'text-rose-400' :
  ...
  return (
    <div className={`border ${borderClass} rounded-xl p-3`}>
      <div className="flex items-start gap-3">
        <div className={`text-xs font-bold uppercase ${textClass} shrink-0 mt-0.5`}>
          {item.priority}
        </div>
        <div className="flex-1 min-w-0">
          ...
          {item.url && (
            <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:text-primary/80 flex items-center gap-1 mt-1">
              {item.url.slice(0, 60)}... <ExternalLink className="w-3 h-3" />
            </a>
          )}
          {item.imageUrl && (
            <a href={item.imageUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:text-primary/80 flex items-center gap-1 mt-1">
              {item.imageUrl.slice(0, 60)}... <ExternalLink className="w-3 h-3" />
            </a>
          )}
        </div>
      </div>
