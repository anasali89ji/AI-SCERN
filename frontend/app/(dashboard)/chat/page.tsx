...
function UserAvatar({ imageUrl, name, size = 'md' }: { imageUrl?: string|null; name?: string|null; size?: 'sm'|'md' }) {
  ...
  return (
    <div className={`${cls} bg-gradient-to-br from-primary to-moss-200 flex items-center justify-center shrink-0 mt-0.5 font-bold text-white`}>
      {initials}
    </div>
  )
}
      ...
      <div className={`flex flex-col gap-1 max-w-[85%] sm:max-w-[78%] min-w-0 ${isUser ? 'items-end' : 'items-start'}`}>
        ...
        {(msg.content || (msg.isStreaming && msg.content)) && (
          <div className={`rounded-2xl px-3 sm:px-4 py-2.5 sm:py-3 text-sm ${
            isUser
              ? 'bg-gradient-to-br from-primary to-moss-200 text-white rounded-br-sm shadow-lg shadow-primary/15'
...
// ── Welcome suggestions ────────────────────────────────────────────────────
const SUGGESTIONS = [
  { Ic: Ico.Image,    text: "Upload an image to verify if it's AI-generated or a deepfake",  cat: 'Image'   },
  { Ic: Ico.FileText, text: 'Paste text to check if it was written by AI',                    cat: 'Text'    },
  { Ic: Ico.Music,    text: 'How does ensemble verification work for voice cloning?',             cat: 'Audio'   },
  ...
  { Ic: Ico.DB,       text: "Show me Aiscern's current verification statistics",                 cat: 'Data'    },
]
    ...
    try {
      ...
      if (unsupportedAtts?.length) {
        setChats(p=>p.map(c=>c.id===chatId?{...c,messages:c.messages.map(m=>m.id===aid?{
          ...m,
          content:`⚠️ Note: Audio/video files can't be analyzed directly in chat. Please use the dedicated [Audio](/detect/audio) or [Video](/detect/video) verification tools.\n\n`,
        }:m)}:c))
      }
        ...
        for(const block of blocks){
          for(const line of block.split('\n')){
            ...
            try{
              ...
              // ── ARIA format (transformed backend) ──────────────────────────
              if(ev.type==='thinking'){
                setChats(p=>p.map(c=>c.id===chatId?{...c,messages:c.messages.map(m=>m.id===aid?{...m,isThinking:true}:m)}:c))
              }
              if(ev.type==='text'){
                setChats(p=>p.map(c=>c.id===chatId?{...c,messages:c.messages.map(m=>m.id===aid?{...m,isThinking:false,content:m.content+ev.text}:m)}:c))
              }
              if(ev.type==='tool_result'){
                setChats(p=>p.map(c=>c.id===chatId?{...c,messages:c.messages.map(m=>m.id===aid?{...m,toolEvents:[...(m.toolEvents||[]).filter(t=>t.tool!==ev.tool),{tool:ev.tool,status:'done',result:ev.result}]}:m)}:c))
              }
              if(ev.type==='done'){
                setChats(p=>p.map(c=>c.id===chatId?{...c,messages:c.messages.map(m=>m.id===aid?{...m,isThinking:false,isStreaming:false}:m),updatedAt:now()}:c))
              }

              // ── RAW NVIDIA format (fallback for stale deployments / direct NIM) ──
              if(ev.choices && ev.choices[0]?.delta?.content){
                const chunk = ev.choices[0].delta.content
                setChats(p=>p.map(c=>c.id===chatId?{...c,messages:c.messages.map(m=>m.id===aid?{...m,isThinking:false,content:m.content+chunk}:m)}:c))
              }

              // ── End-of-stream marker (raw OpenAI/NVIDIA format) ────────────
              if(raw === '[DONE]'){
                setChats(p=>p.map(c=>c.id===chatId?{...c,messages:c.messages.map(m=>m.id===aid?{...m,isThinking:false,isStreaming:false}:m),updatedAt:now()}:c))
              }
    ...
    <div ref={chatContainerRef} className="flex h-[calc(100dvh-4rem)] bg-[#080c14] overflow-hidden">
      ...
      `}>
        <div className="p-3 pt-[calc(1rem+env(safe-area-inset-top,0px))] border-b border-white/[0.06] space-y-2">
          <button onClick={newChat} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-gradient-to-r from-primary to-moss-200 text-white text-sm font-semibold hover:opacity-90 active:scale-[0.97] transition-all shadow-lg shadow-primary/20">
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="text-xs text-gray-600 mt-0.5 hidden sm:block">
                {activeChat ? `${activeChat.messages.length} messages · ${activeChat.title.slice(0,40)}` : 'Multi-modal · Tool-enabled · AI verification specialist'}
              </div>
            </div>
          </div>
        ...
        <div ref={messagesAreaRef} className="flex-1 overflow-y-auto scroll-smooth">
          {!activeChat || activeChat.messages.length===0 ? (
            <div className="min-h-full flex flex-col items-center justify-center px-4 py-5 max-w-2xl 2xl:max-w-3xl mx-auto w-full">
              ...
              <p className="text-gray-500 text-[10px] font-bold uppercase tracking-[0.15em] mb-2">Aiscern AI Verification Assistant</p>
              <p className="text-gray-600 text-xs sm:text-sm text-center mb-4 max-w-sm leading-relaxed">
                Ask anything about AI verification, upload media for deepfake analysis, or explore Aiscern's capabilities.
              </p>

              <div className="hidden xs:flex flex-wrap justify-center gap-1.5 mb-3">
                {[['Text Verification',Ico.FileText],['Deepfake Analysis',Ico.Image],['Voice Cloning',Ico.Music],['Video Deepfakes',Ico.Video],['General Q&A',Ico.Globe],['Dataset Insights',Ico.DB]].map(([l,I])=>{
            ...
            <div className="flex items-end gap-2 px-3 py-3 rounded-2xl border border-white/[0.09] bg-[#0d1117] focus-within:border-primary/50 focus-within:shadow-xl focus-within:shadow-primary/8 transition-all duration-200">
              ...
              {loading
                ...
                : <button
                    ...
                    className="p-2.5 rounded-xl bg-gradient-to-br from-primary to-moss-200 text-white disabled:opacity-25 hover:opacity-90 active:scale-95 transition-all shrink-0 shadow-lg shadow-primary/20"
                  >
                    <Ico.Send />
                  </button>
              }
