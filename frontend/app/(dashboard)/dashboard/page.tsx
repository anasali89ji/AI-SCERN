...
const TOOLS = [
  { href: '/detect/text',  icon: FileType2,   label: 'Text',  desc: 'Verify AI writing'      },
  { href: '/detect/image', icon: ImageIcon,  label: 'Image', desc: 'Deepfake verification'      },
  { href: '/detect/audio', icon: Music,      label: 'Audio', desc: 'Voice clone verification'   },
  { href: '/detect/video', icon: Video,      label: 'Video', desc: 'Deepfake video verification' },
  { href: '/batch',        icon: Layers,     label: 'Batch', desc: 'Analyze 20 files at once'   },
  { href: '/chat',         icon: Bot,        label: 'ARIA',  desc: 'AI verification assistant'  },
]
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="animate-enter">
        ...
        <p className="text-[#6B6B6B] text-sm mt-1">
          {totalScans === 0
            ? 'Run your first verification below — completely free.'
            : `You've run ${totalScans.toLocaleString()} verification${totalScans !== 1 ? 's' : ''} so far.`}
        </p>
      </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-[#A3A3A3] mb-4 leading-relaxed">
                Pick a verification tool to run your first scan. Free — 10 scans per day on text and image.
              </p>
              <div className="flex flex-wrap gap-2">
                <Link href="/detect/text"
                  ...
                  <FileType2 className="w-3.5 h-3.5" /> Try AI Text Verification
                </Link>
                <Link href="/detect/image"
                  ...
                  <ImageIcon className="w-3.5 h-3.5" /> Try Deepfake Image Verification
                </Link>
              </div>
            </div>
          </div>
      ...
      <div className="space-y-2">
        <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[#6B6B6B] mb-3">AI Verification Tools</p>
      ...
      {totalScans > 0 && (
        <div className="p-5 rounded-xl bg-[#141414] border border-[#333333]">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-white">Verification Credits</h2>
      ...
      <div className="space-y-3">
        ...
        {loading ? (
          ...
          <div className="p-8 rounded-xl bg-[#141414] border border-[#333333] text-center">
            ...
            <p className="text-[#A3A3A3] text-sm font-medium mb-1">Couldn't load verification history</p>
          ...
          <div className="p-10 rounded-xl bg-[#141414] border border-[#333333] text-center">
            ...
            <p className="text-[#6B6B6B] text-xs mb-5">Pick a tool above to run your first verification</p>
            <Link href="/detect/text"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl
                         bg-[#2BEE34] hover:bg-[#1A8F1F] text-[#0A0A0A] text-sm font-semibold transition-colors">
              <FileType2 className="w-4 h-4" /> Try AI Text Verification
            </Link>
          </div>
        ) : (
          <div className="space-y-2">
