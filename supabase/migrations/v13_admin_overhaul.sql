...
-- NOTE: This project uses Supabase Auth (auth.users).
--       There is NO custom "users" table.
--       user_id columns are TEXT (Clerk/Supabase user IDs).
-- ============================================================
...
-- ── 4. support_tickets ───────────────────────────────────────
-- user_id is TEXT to match the rest of the schema (no FK to auth.users
-- because Clerk user IDs are strings, not Postgres UUIDs)
...
-- ── 5. error_logs — extend if already exists ─────────────────
...
-- ── 7. feature_flags — add missing columns ───────────────────
...
ALTER TABLE feature_flags ADD COLUMN IF NOT EXISTS target_plans        TEXT[]  DEFAULT '{}';
ALTER TABLE feature_flags ADD COLUMN IF NOT EXISTS updated_at          TIMESTAMPTZ;

-- Back-fill name from key for any existing rows
...
-- ── 9. api_keys — extend existing table ──────────────────────
...
-- Back-fill key_prefix from key_hash or id
UPDATE api_keys
...
-- ── 10. payment_events ───────────────────────────────────────
-- user_id TEXT — matches scans/batch_jobs style, no FK
...
-- ── 13. RLS ───────────────────────────────────────────────────
-- All admin-only tables: block public, service role bypasses automatically
...
-- Block all direct client access to admin tables
DO $$
DECLARE tbl TEXT;
BEGIN
  FOREACH tbl IN ARRAY ARRAY[
    ...
    EXECUTE format('DROP POLICY IF EXISTS block_public ON %I', tbl);
    EXECUTE format('CREATE POLICY block_public ON %I FOR ALL TO PUBLIC USING (false)', tbl);
  END LOOP;
END $$;

-- announcements: authenticated users can read active ones
...
-- support_tickets: users manage only their own rows
