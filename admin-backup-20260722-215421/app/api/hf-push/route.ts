...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const stats = items.reduce((a: any, i: any) => {
      const k = `${i.media_type}_${i.label}`
      a[k] = (a[k] || 0) + 1
      return a
    }, {})
    ...
    if (!HF_TOKEN) {
      return NextResponse.json({
        ...
        message: 'Set HUGGINGFACE_API_TOKEN env var for real push',
      })
    }
    ...
    const commitRes = await fetch(`https://huggingface.co/api/datasets/${HF_DATASET_REPO}/commit/main`, {
      ...
      body: JSON.stringify({
        ...
        files: [{ path: filename, content: Buffer.from(jsonl).toString('base64') }],
      }),
    })
    ...
    if (!commitRes.ok) {
      ...
      if (commitRes.status === 404) {
        return NextResponse.json({
          ...
          total: items.length,
          stats,
          message: `Repo "${HF_DATASET_REPO}" not found on HuggingFace. Create it first.`,
        })
      }
    ...
    await db.from('dataset_items').update({ hf_dataset_id: HF_DATASET_REPO })
      .eq('is_deduplicated', true)
      .is('hf_dataset_id', null)

    await logAdminAction('hf_push', null, auth.ip, { media_type, total: items.length }, auth.adminId)
